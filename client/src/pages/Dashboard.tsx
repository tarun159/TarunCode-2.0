...
export function Dashboard() {
  ...
  return (
    <div className="relative">
      {/* Holographic energy field — purely decorative background. */}
      <LabBackground activeLab={activeLab} />
          ...
          {labs.map((lab, i) => {
            ...
            return (
              <Link
                key={lab.to}
                to={lab.to}
                onMouseEnter={() => setActiveLab(lab.key)}
                onMouseLeave={() => setActiveLab(null)}
                onFocus={() => setActiveLab(lab.key)}
                onBlur={() => setActiveLab(null)}
                onTouchStart={() => setActiveLab(lab.key)}
                onTouchEnd={() => setActiveLab(null)}
              >
                <motion.div
